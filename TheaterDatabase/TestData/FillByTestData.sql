...
SELECT @ConnectionStrings =
 BulkColumn
 FROM OPENROWSET(BULK N'$(FullScriptDir)\TheaterSchedule\appsettings.json', SINGLE_BLOB) JSON

 IF (ISJSON(@ConnectionStrings) = 1)
		BEGIN
        PRINT 'JSON is valid';
...
SET @indexstart = (SELECT CHARINDEX('=', @value));  
SET @indexend = (SELECT LEN(@value));
SET @Database = SUBSTRING(@value, @indexstart + 1, @indexend);
SELECT @Database;
...
EXECUTE(@Language);
EXECUTE(@Performance);
EXECUTE(@PerformanceCreativeTeamMember);
EXECUTE(@Schedule);
EXECUTE(@Settings);
EXECUTE(@Account);
EXECUTE(@Message);
